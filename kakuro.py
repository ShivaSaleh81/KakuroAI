'''backtracking with improved heuristic function '''
import time
start = time.time()
...
def is_okay(cell_key):
    ...
    for c in cells[cell_key].col_guide.cells:
        ...
        if c.value == 0:
            ...
            sum += c.value
    sum += cells[cell_key].value
    if sum < cells[cell_key].col_guide.value:
        return False
    if zero_count == 0 and sum > cells[cell_key].col_guide.value:
        return False
    for c in cells[cell_key].col_guide.cells: 
        x = 0
    ...
    for c in cells[cell_key].row_guide.cells:
        ...
        if c.value == 0:
            ...
            sum += c.value
    sum += cells[cell_key].value
    if sum < cells[cell_key].row_guide.value:
        return False
    if zero_count == 0 and sum > cells[cell_key].row_guide.value:
        return False
    for c in cells[cell_key].row_guide.cells: 
        x = 0
